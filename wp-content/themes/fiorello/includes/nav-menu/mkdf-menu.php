...
if ( ! function_exists( 'fiorello_mikado_add_custom_nav_fields' ) ) {
	function fiorello_mikado_add_custom_nav_fields( $menu_item ) {
	...
	add_filter( 'wp_setup_nav_menu_item', 'fiorello_mikado_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'fiorello_mikado_update_custom_nav_fields' ) ) {
	function fiorello_mikado_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'fiorello_mikado_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'fiorello_mikado_edit_walker' ) ) {
	function fiorello_mikado_edit_walker( $walker, $menu_id ) {
		return 'FiorelloMikado_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'fiorello_mikado_edit_walker', 10, 2 );
}

if ( ! function_exists( 'fiorello_mikado_include_custom_walkers' ) ) {
	function fiorello_mikado_include_custom_walkers() {
		...
		do_action( 'fiorello_mikado_action_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'fiorello_mikado_include_custom_walkers' );
}