...
class MikadoSkin extends FiorelloMikadoSkinAbstract {
    /**
     * Skin constructor. Hooks to fiorello_mikado_admin_scripts_init and fiorello_mikado_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'mikado';

        //hook to
        add_action('fiorello_mikado_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('fiorello_mikado_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('fiorello_mikado_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('fiorello_mikado_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('fiorello_mikado_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('fiorello_mikado_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			fiorello_mikado_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            fiorello_mikado_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    		= fiorello_mikado_get_admin_tab();
        $active_page 	= $fiorello_mikado_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see FiorelloMikadoSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param FiorelloMikadoAdminPage $page current page to load
	...
	public function renderBackupOptions() { ?>
		<div class="mkdf-options-page mkdf-page">
			<?php $this->getHeader(fiorello_mikado_get_theme_info_item('Name'), fiorello_mikado_get_theme_info_item('Version'), false); ?>
			<div class="mkdf-page-content-wrapper">
				<div class="mkdf-page-content">
					<div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
