...
class FiorelloMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_fiorello_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_fiorello_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'fiorello' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'fiorello' );
		}
...
if ( ! function_exists( 'fiorello_mikado_create_like' ) ) {
	function fiorello_mikado_create_like() {
		FiorelloMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'fiorello_mikado_create_like' );
}