<?php
namespace FiorelloCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Product Info', 'fiorello' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'fiorello' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'fiorello' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'fiorello' ),
							'value'       => array(
								esc_html__( 'Title', 'fiorello' )          => 'title',
								esc_html__( 'Featured Image', 'fiorello' ) => 'featured_image',
								esc_html__( 'Category', 'fiorello' )       => 'category',
								esc_html__( 'Excerpt', 'fiorello' )        => 'excerpt',
								esc_html__( 'Price', 'fiorello' )          => 'price',
								esc_html__( 'Rating', 'fiorello' )         => 'rating',
								esc_html__( 'Add To Cart', 'fiorello' )    => 'add_to_cart',
								esc_html__( 'Tag', 'fiorello' )            => 'tag',
								esc_html__( 'Author', 'fiorello' )         => 'author',
								esc_html__( 'Date', 'fiorello' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'fiorello' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'fiorello' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'fiorello' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'fiorello' ),
							'value'       => array_flip( fiorello_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'fiorello' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'fiorello' ),
							'value'      => array(
								esc_html__( 'Default', 'fiorello' ) => '',
								esc_html__( 'White', 'fiorello' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'fiorello' ),
							'value'      => array(
								esc_html__( 'Default', 'fiorello' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'fiorello' ) ),
							'group'      => esc_html__( 'Product Info Style', 'fiorello' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="mkdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "fiorello" ), $average ) . '" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'fiorello_mikado_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . fiorello_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'fiorello' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'fiorello' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'fiorello' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'fiorello' ) . ': ' . $product_id;
