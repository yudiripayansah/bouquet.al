...
if ( ! function_exists( 'fiorello_mikado_get_icon_pack_collections' ) ) {
	function fiorello_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'fiorello_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'fiorello_mikado_get_icon_pack_collections' );
}

if ( ! function_exists( 'fiorello_mikado_get_icon_pack_options' ) ) {
	function fiorello_mikado_get_icon_pack_options() {
		$options = apply_filters( 'fiorello_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'fiorello_mikado_get_icon_pack_options' );
}

if ( ! function_exists( 'fiorello_mikado_get_icon_pack_social_options' ) ) {
	function fiorello_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'fiorello_mikado_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'fiorello_mikado_get_icon_pack_social_options' );
}

/*
 * Class: FiorelloMikadoIconCollections
 */
class FiorelloMikadoIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = fiorello_mikado_get_icon_pack_collections( true );
		$this->iconPackOptions       = fiorello_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = fiorello_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = fiorello_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'fiorello' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'fiorello' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'fiorello' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'fiorello' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'fiorello' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'fiorello' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'fiorello' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'fiorello' )       => 'fa-lg',
			esc_html__( 'Small', 'fiorello' )      => 'fa-2x',
			esc_html__( 'Medium', 'fiorello' )     => 'fa-3x',
			esc_html__( 'Large', 'fiorello' )      => 'fa-4x',
			esc_html__( 'Very Large', 'fiorello' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iFiorelloMikadoIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'fiorello' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'fiorello' ) ] = '';
