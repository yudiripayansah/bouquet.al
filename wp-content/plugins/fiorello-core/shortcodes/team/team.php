<?php
namespace FiorelloCore\CPT\Shortcodes\Team;
...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = fiorello_mikado_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'fiorello-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'fiorello-core' ) . $x . esc_html__( ' Link', 'fiorello-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => fiorello_mikado_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'fiorello-core' ) . $x . esc_html__( ' Target', 'fiorello-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'fiorello-core' ) => '_self',
					esc_html__( 'New Window', 'fiorello-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Mikado Team', 'fiorello-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'fiorello-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'fiorello-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Name', 'fiorello-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'fiorello-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'fiorello-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
							...
							array(
								...
								'heading'    => esc_html__( 'Position Color', 'fiorello-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'fiorello-core' ),
								'value'       => array_merge( array( '' => '' ), fiorello_mikado_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( fiorello_mikado_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = fiorello_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = fiorello_mikado_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = fiorello_mikado_execute_shortcode( 'mkdf_icon', $team_icon_params );
				}
			}
		}
